...
def Login(request):
    if request.method == 'POST':
        ...
        try:
            ...
            for lines in reapons:
                if compare(lines.user_password, user_password):  # 用户输入密码正确
                    state = "OK"  # 记录查询反馈信息，OK代表查询成功，False代表查询失败
                    information = 1  # 记录查询详细信息，若查询未出错，则为1；若出错。则记录出错详细信息，类似于控制台报错信息
                    login_or_not = True  # 记录用户登录成功，将返回
                    break
            else:
                state = 'OK'
                information = 1
                login_or_not = False  # 用户输入密码错误，登录失败
